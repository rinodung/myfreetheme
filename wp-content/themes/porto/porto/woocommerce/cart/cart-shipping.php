	...
	<td>
		<?php if ( ! empty( $available_methods ) ) : ?>
			...
			<?php elseif ( get_option( 'woocommerce_shipping_method_format' ) === 'select' ) : ?>
				...
				<select name="shipping_method[<?php echo $index; ?>]" data-index="<?php echo $index; ?>" id="shipping_method_<?php echo $index; ?>" class="form-control shipping_method">
		...
		<?php else : ?>
			...
			<?php if ( is_cart() ) : ?>
				...
				<?php echo apply_filters( 'woocommerce_cart_no_shipping_available_html',
					'<div class="alert alert-info woocommerce-info"><p>' . __( 'There doesn&lsquo;t seem to be any available shipping methods. Please double check your address, or contact us if you need any help.', 'woocommerce' ) . '</p></div>'
				); ?>
